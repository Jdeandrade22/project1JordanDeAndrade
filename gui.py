...
def save_user_details(user_details):
    ...
    ''', (
        ...
        ', '.join(user_details["projects"]),  # Convert list to string
...
def load_user_details(user_id, window):
    ...
    conn = sqlite3.connect('savedJobs.db')
    cursor = conn.cursor()
    cursor.execute("SELECT name, email, phone, github_linkedin, projects, classes, other FROM user_details WHERE id=?", (user_id,))
